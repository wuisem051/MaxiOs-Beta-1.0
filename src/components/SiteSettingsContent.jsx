...
const SiteSettingsContent = () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          try {
            ...
            setError('Error al crear la configuración por defecto del sitio.');
          }
        }
      } catch (err) {
  ...
  const handleSaveSettings = async (e) => {
    ...
    try {
      ...
      setMessage('Configuración del sitio guardada exitosamente!');
    } catch (err) {
      console.error("Error saving site settings:", err);
      setError(`Fallo al guardar la configuración: ${err.message}`);
    } finally {
      setLoading(false);
  ...
  const handleResetPerformanceStats = async () => {
    ...
    try {
      ...
      setMessage('Estadísticas de rendimiento reiniciadas exitosamente!');
    } catch (err) {
      console.error("Error resetting performance stats:", err);
      setError(`Fallo al reiniciar estadísticas: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-800 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-white mb-6">Configuración del Sitio</h2>

      {loading && <p className="text-gray-400 mb-4">Cargando configuración...</p>}
      {error && <div className="bg-red-500 text-white p-3 rounded mb-4">{error}</div>}
      {message && <div className="bg-green-500 text-white p-3 rounded mb-4">{message}</div>}

      <form onSubmit={handleSaveSettings} className="space-y-6">
        <div>
          <label htmlFor="siteName" className="block text-gray-300 text-sm font-medium mb-2">Nombre del Sitio</label>
          <input
            type="text"
            id="siteName"
            value={siteName}
            onChange={(e) => setSiteName(e.target.value)}
            className="w-full p-3 bg-gray-700 border border-gray-600 rounded-md text-white text-base focus:outline-none focus:border-yellow-500"
            placeholder="Ej: Mi Plataforma de Minería"
            required
          />
        </div>
        <div>
          <label htmlFor="heroTitle" className="block text-gray-300 text-sm font-medium mb-2">Título Principal (Hero)</label>
          <input
            type="text"
            id="heroTitle"
            value={heroTitle}
        ...
        <div>
          <label htmlFor="homeText" className="block text-gray-300 text-sm font-medium mb-2">Texto de la Página de Inicio (Home)</label>
          <textarea
            id="homeText"
            rows="4"
            value={homeText}
        ...
        <div>
          <label htmlFor="siteDomain" className="block text-gray-300 text-sm font-medium mb-2">Dominio del Sitio Web</label>
          <input
            type="text"
            id="siteDomain"
            value={siteDomain}
        ...
        <div>
          <label htmlFor="faviconUrl" className="block text-gray-300 text-sm font-medium mb-2">URL del Favicon</label>
          <input
            type="text"
            id="faviconUrl"
            value={faviconUrl}
          ...
          />
          {faviconUrl && (
            <p className="text-gray-400 text-sm mt-2">Favicon actual: <a href={faviconUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Ver Favicon</a></p>
          )}
          <label htmlFor="faviconUpload" className="block text-gray-300 text-sm font-medium mb-2 mt-4">Subir Favicon (.ico, .png) - (Funcionalidad de subida directa no disponible en esta migración)</label>
          <input
            type="file"
            id="faviconUpload"
            accept=".ico,.png"
            onChange={handleFileChange}
            className="w-full p-3 bg-gray-700 border border-gray-600 rounded-md text-white text-base focus:outline-none focus:border-yellow-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-yellow-50 file:text-yellow-700 hover:file:bg-yellow-100"
            disabled // Deshabilitar la subida directa de archivos
          />
        </div>
        <div>
          <label htmlFor="footerText" className="block text-gray-300 text-sm font-medium mb-2">Texto del Pie de Página (Footer)</label>
          <textarea
            id="footerText"
            rows="3"
            value={footerText}
      ...
      <div className="mt-8 pt-6 border-t border-gray-700">
        <h3 className="text-xl font-bold text-white mb-4">Herramientas de Administración</h3>
        <button
